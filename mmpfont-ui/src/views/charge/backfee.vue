  ...
  <div v-loading="loading" class="app-container" :element-loading-text="loadingText">
    ...
    <div v-if="careHistory.regId!==undefined" style="float: left">
      ...
      <el-card style="margin-bottom: 5px">
        ...
        <el-button type="danger" icon="el-icon-bank-card"  @click="handleBackfeeWithZfb">微信退费</el-button>
...
export default {
  ...
  methods: {
    ...
    handleQuery(){
      ...
      this.$axios.get("charge/api/hisOrderBackfee/list/"+this.regId).then(result=>{
    ...
    handleBackfeeWithCash() {
      if (!this.careHistory.regId) {
        ...
        this.$confirm('是否确定创建订单并使用现金退费?', '提示', {
          ...
          // 发送现金退费的请求
          this.$axios.post("charge/api/hisOrderBackfee/updateBystatus",postObj).then(res => {
            this.resetCurrentParams()
            this.$message.success('现金退费成功')
            this.loading = false
          }).catch(() => {
    ...
    handleBackfeeWithZfb() {
      if (!this.careHistory.regId) {
        ...
        // 发送请求
        this.loading = true
        this.loadingText = '订单创建并微信退费中'
        this.$confirm('是否确定创建订单并使用微信退纲?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
