<template>
  <div  class="app-container">
    ...
    <el-card class="cardmargin">
      <el-form ref="queryParams" :model="this.queryParams" :inline="true" label-width="68px">
  ...
  data() {
    return {
      ...
      revenueOverview: { // 收支概况
        title: '收支概况',
        data: [
          { value:0.00, name: '收费金额' },
          { value:0.00, name: '退费金额' }
        ]
      },
      incomeChanel: { // 收入渠道
        title: '收入渠道',
        data: [
          { value:0.00, name: '现金收入' },
          { value:0.00, name: '支付宝收入' }
        ]
      },
      refund: { // 退款金额和渠道
        title: '退款',
        data: [
          { value:0.00, name: '现金退费' },
          { value:0.00, name: '支付宝退费' }
        ]
      }
    }
  },
    ...
    loadData() {
      ...
      this.$axios.post('statistics/api/revenue/getInfo',qs.stringify(param)).then(res => {
        ...
        let revenueOverview = {
          title: '收支概况',
          data: [{
            value: res.data.t.toll,
            name: '收费金额'
          },
            {
              value: res.data.t.refund,
              name: '退费金额'
            }
          ]
        }
        let incomeChanel = {
          title: '收入渠道',
          data: [{
            value: res.data.t.cashIncome,
            name: '现金笔数'
          },
            {
              value: res.data.t.alipayIncome,
              name: '支付宝笔数'
            }
          ]
        }
        let refund = {
          title: '退款',
          data: [{
            value: res.data.t.cashRefund,
            name: '现金退费'
          },
            {
              value: res.data.t.alipayRefund,
              name: '支付宝退费'
            }
          ]
        }
        this.revenueOverview = revenueOverview;
        this.incomeChanel = incomeChanel;
        this.refund = refund;
