  ...
  <div class="app-container">
    ...
    <el-card class="box-card">
      <el-table :data="purchaseItemList" class="tb-edit" style="width:100%" border highlight-current-row>
        ...
        <el-table-column label="数量" align="center" width="210px" prop="purchaseNumber">
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.purchaseNumber" min="0" :step="1" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="批发价" align="center" width="210px" prop="tradePrice">
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.tradePrice" min="0" :step="0.1" :precision="2" />
          </template>
        </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="1100px" center append-to-body>
      ...
      <el-table ref="medicinesTableList" v-loading="loading" border :data="medicinesTableList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="生产厂家" width="240px" align="center" prop="producterId" :formatter="ProducterNameDict"/>
...
export default {
  ...
  methods: {
      ...
      if (this.purchaseItemList.length > 0) {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            this.$axios.post("/stock/api/purchase/addPurchase", purchaseAllVo).then(res => {
      ...
      if (this.purchaseItemList.length > 0) {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            // 组装要提交到后台的数据
            const purchaseAllVo = { 'purchaseDto': this.form, 'purchaseItemDtos': this.purchaseItemList }
            this.$axios.post("/stock/api/purchase/addPurchaseToAudit", purchaseAllVo).then(res => {
    ...
    // 查询供应商的数据
    selectAllProvider(){
      this.$axios.get("/stock/api/provider/getAllDict").then(result => {
        this.providerOptions = result.data.t;
      })
    },
    // 加载单据号
    generatePurchaseId(){
      this.$axios.get("/stock/api/purchase/generatePurchaseId").then(result => {
        this.form.purchaseId = result.data.t;
      })
    },
