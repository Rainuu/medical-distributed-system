...
export default {
  ...
  methods: {
    // 查询表格数据
    getRegisteredTtemList() {
      this.loading = true // 打开遮罩
      this.$axios.post("system/api/RegisteredTtem/getAll",qs.stringify(this.queryParams)).then(res => {
        this.registeredItemTableList= res.data.t.records
    ...
    // 重置查询条件
    resetQuery() {
      this.queryParams.regItemName=undefined
    ...
    selectUp(){
      this.handleUpdate(this.selectData)
    },
    // 打开修改的弹出层
    handleUpdate(row) {
      this.title = '修改检查项目信息'
    ...
    // 执行删除
    piliDelete(row) {
      const RegisteredTtemIds = this.ids
      const tx = this
    ...
    // 执行删除
    handleDelete(row) {
      const RegisteredTtemIds = row.regItemId
      const tx = this
      tx.$confirm('此操作将永久删除该检查项目数据, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (tx.form.regItemId === undefined) {
            this.$axios.post("system/api/RegisteredTtem/saveAndUp",qs.stringify(this.form)).then(res => {
              tx.$message.success('保存成功')
              tx.loading = false
              tx.getRegisteredTtemList()// 列表重新查询
              tx.open = false// 关闭弹出层
            }).catch(() => {
              tx.loading = false
            })
